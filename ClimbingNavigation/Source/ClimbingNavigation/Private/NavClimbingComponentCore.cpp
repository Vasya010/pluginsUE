...
void UNavClimbingComponentCore::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (bPrintAditionalInfoOnScreen)
	{
		...
		if (const UEnum* EnumPtr = StaticEnum<ENavClimbActionState>())
		{
			const int64 Value = static_cast<int64>(NavClimbActionState);

			const FString EnumValue = EnumPtr->GetNameByValue(Value).ToString();
			const FString DisplayName = EnumPtr->GetDisplayNameTextByValue(Value).ToString();

			S1 += DisplayName;

			if (GEngine)
			{
				GEngine->AddOnScreenDebugMessage(13, 0.0f, FColor::Cyan, S1, false);
			}
		}
