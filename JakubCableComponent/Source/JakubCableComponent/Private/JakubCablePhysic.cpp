...
public:
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateIndex<int32>(TEXT("FJCableIndexBuffer"), NumIndices * 2)
...
public:
	...
	{
		VertexBuffers.InitWithDummyData(&UE::RenderCommandPipe::JakubCable, &VertexFactory, GetRequiredVertexCount());
	...
	void BuildCableMesh(FRHICommandListBase& RHICmdList, const FCableDynamicData* NewDynamicData)
	{
		...
		const int32 NumRingVerts = NumSides + 1;
		const int32 ExpectedVertCount = GetRequiredVertexCount();
		check(VertexBuffers.PositionVertexBuffer.GetNumVertices() == ExpectedVertCount);
		...
		{
			// kierunki
			const FVector ForwardDir = (InPoints[SectionIdx + 1] - InPoints[SectionIdx]).GetSafeNormal();
			...
			// dwa pierścienie na segment
			for (int32 RingIdx = 0; RingIdx < 2; ++RingIdx)
			{
				const FVector RingPosition = FMath::Lerp(
					InPoints[SectionIdx],
					InPoints[SectionIdx + 1],
					(float)RingIdx);

				const float AlongFrac =
					((float)SectionIdx + (float)RingIdx) / (float)SegmentCount;

				const int32 AlongIdx = SectionIdx * 2 + RingIdx;   // <── ważne

				for (int32 AroundIdx = 0; AroundIdx < NumRingVerts; ++AroundIdx)
				{
					const float AroundFrac = float(AroundIdx) / float(NumSides);
					const float RadAngle = 2.f * PI * AroundFrac;

					const FVector OutDir =
						(FMath::Cos(RadAngle) * UpDir) +
						(FMath::Sin(RadAngle) * RightDir);

					const int32 VertIndex = GetVertIndex(AlongIdx, AroundIdx); // <── tu klucz

					VertexBuffers.PositionVertexBuffer.VertexPosition(VertIndex) =
						FVector3f(RingPosition + (OutDir * 0.5f * CableWidth));

					VertexBuffers.StaticMeshVertexBuffer.SetVertexUV(
						VertIndex, 0, FVector2f(AlongFrac * TileMaterial, AroundFrac));

					VertexBuffers.ColorVertexBuffer.VertexColor(VertIndex) = VertexColor;

					VertexBuffers.StaticMeshVertexBuffer.SetVertexTangents(
						VertIndex,
						(FVector3f)ForwardDir,
						FVector3f(OutDir ^ ForwardDir),
						(FVector3f)OutDir);
		...
		check(WrittenVerts == ExpectedVertCount);

		// 3) Skopiuj CPU-owe buforu do RHI (dokładnie jak wcześniej w SetDynamicData_RenderThread)
		{
		...
		for (int32 SegIdx = 0; SegIdx < SegmentCount; SegIdx++)
		{
			for (int32 SideIdx = 0; SideIdx < NumSides; SideIdx++)
			{
				...
				if (SegIdx < SegmentCount - 1) {
					IndexBufferData[IndexBufferIndex++] = TR;
					IndexBufferData[IndexBufferIndex++] = BR;
					IndexBufferData[IndexBufferIndex++] = TR2;

					IndexBufferData[IndexBufferIndex++] = TR2;
					IndexBufferData[IndexBufferIndex++] = BR;
					IndexBufferData[IndexBufferIndex++] = BR2;
				}
			}
		}
...
void UJakubCablePhysic::SendRenderDynamicData_Concurrent()
{
	if (SceneProxy)
	{
		...
		ENQUEUE_RENDER_COMMAND(FSendCableDynamicData)(UE::RenderCommandPipe::JakubCable,
