	...
	: Super(ObjectInitializer)
{
	...
	struct FConstructorStatics
	{
		...
		ConstructorHelpers::FObjectFinder<UCurveFloat> SunElevationIntensityCurve;
		ConstructorHelpers::FObjectFinder<UCurveFloat> MoonElevationIntensityCurve;
		...
		FConstructorStatics()
			...
			, SunElevationIntensityCurve(TEXT("UCurveFloat'/SkyCreatorPlugin/Curves/Curve_SunElevationIntensityScale'"))
			, MoonElevationIntensityCurve(TEXT("UCurveFloat'/SkyCreatorPlugin/Curves/Curve_MoonElevationIntensityScale'"))
		{
		}
	};
...
//	PrimaryActorTick.bAllowTickOnDedicatedServer = true;
	...
	SunElevationIntensityScaleCurve = ConstructorStatics.SunElevationIntensityCurve.Object;
	MoonElevationIntensityScaleCurve = ConstructorStatics.MoonElevationIntensityCurve.Object;
...
//			RealtimeVolumetricCloudWind(DeltaTime);
		...
		if (!GetWorldTimerManager().IsTimerActive(LightningFlashFadeTimerHandle) && USkyCreatorFunctionLibrary::IsApplicationForegroundNow() && !UKismetSystemLibrary::IsDedicatedServer(this))
		{
...
{
	if (GetWorld() && GetWorldTimerManager().TimerExists(LightningFlashFadeTimerHandle))
	{
...
void ASkyCreator::SetComponentsSettings()
{
	...
	{
		SunLight->SetMobility(SunLightMobility);
		SunLight->SetIntensity(GetSunLightIntensity());
...
//		SunLight->DisabledBrightness = SunBrightness;
	...
	{
		MoonLight->SetMobility(MoonLightMobility);
		MoonLight->SetIntensity(GetMoonLightIntensity());
...
void ASkyCreator::SetSunLightSettings(FSkyCreatorSunLightSettings InSunLightSettings)
{
	if (SunLight && CommonMPC)
	{
		...
		SunLight->AtmosphereSunDiskColorScale = GetValueSource(SunAtmosphereDiskColorScaleSource, SunAtmosphereDiskColorScale, WeatherSettings.SunLightSettings.AtmosphereDiskColorScale);
...
//		MoonLight->AtmosphereSunDiskColorScale = WeatherSettings.MoonLightSettings.AtmosphereDiskColorScale;
		MoonLight->AtmosphereSunDiskColorScale = GetValueSource(MoonAtmosphereDiskColorScaleSource, MoonAtmosphereDiskColorScale, WeatherSettings.MoonLightSettings.AtmosphereDiskColorScale);
...
{
	FCelestialPositionData LocalSunPositionData;
	FRotator Result;
	...
	if (SunPositionType == SunPositionType_Simple)
	{
		...
		LocalSunPositionData.Elevation = -SunLight->GetComponentRotation().Pitch;
		LocalSunPositionData.Azimuth = SunLight->GetComponentRotation().Yaw + 180;
		Result = UKismetMathLibrary::ComposeRotators(FRotator(SunAngle, 0, 0), FRotator(0, SunAzimuth, SunElevation - 90.0f));
	}
	else
	{
		LocalSunPositionData = USkyCreatorFunctionLibrary::GetRealSunPosition(Latitude, Longitude, TimeZone, bDaylightSavingTime, Date);
...
//	}
	...
	SunPositionData = LocalSunPositionData;
...
FSkyCreatorMaterialFXSettings ASkyCreator::GetMaterialFXSettings() const
{
...
//	SunCurrentElevation = SunLight->GetRelativeRotation().Pitch;
	float SunCurrentElevation = -SunPositionData.Elevation;
...
//			SunLightIntensityModulated = FMath::InterpEaseInOut(0.0f, SunIntensity, FMath::GetMappedRangeValueClamped(FVector2D(IntensityModulationStartSunAngle, IntensityModulationEndSunAngle), FVector2D(1, 0), SunCurrentElevation), 2.0f);
...
//			MoonLightIntensityModulated = FMath::InterpEaseInOut(MoonIntensity, 0.0f, FMath::GetMappedRangeValueClamped(FVector2D(IntensityModulationEndSunAngle, IntensityModulationStartSunAngle), FVector2D(0, 1), SunCurrentElevation), 2.0f);
		}
		else if (SunCurrentElevation < TransitionStartSunAngle)
		{
			SunSurfaceBrightness = 1.0f;
//			SunLightIntensityModulated = SunIntensity;
			MoonSurfaceBrightness = 0.0f;
//			MoonLightIntensityModulated = 0.0f;
		}
		else if (SunCurrentElevation > TransitionEndSunAngle)
		{
			SunSurfaceBrightness = 0.0f;
//			SunLightIntensityModulated = 0.0f;
			MoonSurfaceBrightness = 1.0f;
//			MoonLightIntensityModulated = MoonIntensity;
		}
...
float ASkyCreator::GetSunLightIntensity()
{
	float LocalSunLightIntensity = GetValueSource(SunIntensitySource, SunIntensity, WeatherSettings.SunLightSettings.Intensity);

	if (bUseSunElevationIntensityScale && SunElevationIntensityScaleCurve)
	{
		SunCalculatedIntensity = LocalSunLightIntensity * SunElevationIntensityScaleCurve->GetFloatValue(SunPositionData.Elevation);
	}
	else
	{
		SunCalculatedIntensity = LocalSunLightIntensity;
	}

	SunCalculatedIntensity *= WeatherSettings.SunLightSettings.IntensityScale;

	return SunCalculatedIntensity;
}

float ASkyCreator::GetMoonLightIntensity()
{
	float LocalMoonLightIntensity = GetValueSource(MoonIntensitySource, MoonIntensity, WeatherSettings.MoonLightSettings.Intensity);

	if (bMoonPhaseLightIntensityScale)
	{
		float ScaledMoonLightIntensity = FMath::InterpSinInOut(LocalMoonLightIntensity * MoonPhaseLightIntensityMaxScale, LocalMoonLightIntensity * MoonPhaseLightIntensityMinScale, MoonPhase * 2);
		LocalMoonLightIntensity = ScaledMoonLightIntensity;
	}
	
	if (bUseMoonElevationIntensityScale && MoonElevationIntensityScaleCurve)
	{
		MoonCalculatedIntensity = LocalMoonLightIntensity * MoonElevationIntensityScaleCurve->GetFloatValue(SunPositionData.Elevation);
	}
	else
	{
		MoonCalculatedIntensity = LocalMoonLightIntensity;
	}

	MoonCalculatedIntensity *= WeatherSettings.MoonLightSettings.IntensityScale;

	return MoonCalculatedIntensity;
}

void ASkyCreator::UseExposureSettings()
{
	PostProcess->Settings.bOverride_AutoExposureMethod = bUseExposureSettings;
	PostProcess->Settings.bOverride_AutoExposureLowPercent = bUseExposureSettings;
	PostProcess->Settings.bOverride_AutoExposureHighPercent = bUseExposureSettings;
	PostProcess->Settings.bOverride_AutoExposureMinBrightness = bUseExposureSettings;
	PostProcess->Settings.bOverride_AutoExposureMaxBrightness = bUseExposureSettings;
	PostProcess->Settings.bOverride_AutoExposureSpeedUp = bUseExposureSettings;
	PostProcess->Settings.bOverride_AutoExposureSpeedDown = bUseExposureSettings;
	PostProcess->Settings.bOverride_AutoExposureBias = bUseExposureSettings;
	PostProcess->Settings.bOverride_AutoExposureBiasCurve = bUseExposureSettings;
	PostProcess->Settings.bOverride_AutoExposureApplyPhysicalCameraExposure = bUseExposureSettings;
	PostProcess->Settings.bOverride_AutoExposureMeterMask = bUseExposureSettings;
	PostProcess->Settings.bOverride_HistogramLogMin = bUseExposureSettings;
	PostProcess->Settings.bOverride_HistogramLogMax = bUseExposureSettings;
	PostProcess->Settings.bOverride_AutoExposureCalibrationConstant_DEPRECATED = bUseExposureSettings;
	PostProcess->Settings.bOverride_AutoExposureApplyPhysicalCameraExposure = bUseExposureSettings;
}
...
//	UE_LOG(LogTemp, Warning, TEXT("SC Post Edit Change Property"));
}
...
//	UE_LOG(LogTemp, Warning, TEXT("SC Post Edit Change"));
}
#endif
...
//SKYCREATOR_DECLARE_SEQUENCER_SETFUNCTION(FSkyCreatorWeatherSettings, EditorWeatherSettings);
